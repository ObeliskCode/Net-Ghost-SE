...
vec4 pointLight()
{
	...
	float diffuseLight = 1.8f; //doesn't have to be same as mdlFrag
	float proper = max(dot(normal, lightDirection), 0.0f);
	float inverse = max(dot(normal, -lightDirection), 0.0f);
	float diffuseAmount = max(proper,inverse);
